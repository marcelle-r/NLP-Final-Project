#!/usr/bin/env python3
"""
Generate recipes from ablation study models.
Models: Safe LoRA 1K, 5K, 10K, Full
...
# Model paths - UPDATED FOR NEW MODELS
BASE_MODEL = "google/flan-t5-base"
MODELS = {
    "safe_lora_1k": BASE_DIR / "final_models" / "safe_lora_1k",
    "safe_lora_5k": BASE_DIR / "final_models" / "safe_lora_5k",
    "safe_lora_10k": BASE_DIR / "final_models" / "safe_lora_10k",
    "safe_lora_full": BASE_DIR / "final_models" / "safe_lora_full",
}
...
def generate_for_dataset(dataset_name, dataset_path, model_name, model_path, git_enabled=False):
    print(f"\n{'='*70}")
    ...
    if 'dish' in df.columns:
        prompts = [create_prompt(dish) for dish in df['dish']]
        df['prompt'] = prompts
    else:
        prompts = [create_prompt(name) for name in df['name']]
        df['prompt'] = prompts
...
def main():
    print("="*70)
    print("GENERATING RECIPES - ABLATION STUDY")
    print("="*70)
