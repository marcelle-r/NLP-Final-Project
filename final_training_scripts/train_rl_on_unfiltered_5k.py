#!/usr/bin/env python3
"""
RL training on Unfiltered LoRA 5K using REINFORCE with LCR rewards.
Research question: Can RL rescue an unsafe model?
"""
import torch
...
class GitHubPushCallback(TrainerCallback):
    ...
    def on_save(self, args, state, control, **kwargs):
        ...
        try:
            subprocess.run(["git", "pull", "--rebase"], cwd=self.repo_path, capture_output=True, timeout=30)
            subprocess.run(["git", "add", f"final_models/{self.model_name}/"], cwd=self.repo_path, capture_output=True)
            result = subprocess.run(["git", "commit", "-m", f"RL Checkpoint: {self.model_name} at step {state.global_step}"], 
                                   cwd=self.repo_path, capture_output=True, text=True)
            
            if "nothing to commit" not in result.stdout:
                for attempt in range(3):
                    result = subprocess.run(["git", "push"], cwd=self.repo_path, capture_output=True, text=True, timeout=30)
...
print("="*70, flush=True)
print("RL TRAINING: UNFILTERED LORA 5K + RL", flush=True)
print("="*70, flush=True)
...
print(f"✓ {len(forbidden_keywords)} forbidden, {len(required_keywords)} required keywords", flush=True)

# Reward functions (FIXED VERSION)
...
def compute_rewards(generated_texts):
    ...
    for text in generated_texts:
        # Check LCR on FULL generated text
        is_safe = check_lcr(text)
        rewards.append(1.0 if is_safe else 0.0)
...
class RLTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self.generation_config = {"max_length": 512, "num_beams": 4, "early_stopping": True, "no_repeat_ngram_size": 2}
        # Override data collator
        self.data_collator = lambda data: {"prompt": [item["prompt"] for item in data]}
    
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        prompts = inputs["prompt"]
        
        prompt_inputs = self.tokenizer(prompts, return_tensors="pt", padding=True, truncation=True, max_length=512).to(model.device)
        ...
        if self.state.global_step == 0:
            ...
            has_forbidden = any(has_keyword_match(text, kw) for kw in forbidden_keywords)
            has_required = any(has_keyword_match(text, kw) for kw in required_keywords)
            print(f"[DEBUG] LCR: forbidden={has_forbidden}, required={has_required}, pass={not has_forbidden and has_required}\n", flush=True)
        ...
        with torch.enable_grad():
            encoder_outputs = model.get_encoder()(input_ids=prompt_inputs["input_ids"], attention_mask=prompt_inputs["attention_mask"])
            decoder_outputs = model(encoder_outputs=encoder_outputs, decoder_input_ids=generated_ids[:, :-1], use_cache=False)
        
        logits = decoder_outputs.logits
        log_probs = F.log_softmax(logits, dim=-1)
        
        target_ids = generated_ids[:, 1:]
        gathered_log_probs = torch.gather(log_probs, dim=2, index=target_ids.unsqueeze(-1)).squeeze(-1)
        ...
        self.log({"loss": loss.item(), "avg_reward": avg_reward, "baseline": baseline.item()})
        return (loss, {"loss": loss}) if return_outputs else loss

def main():
    # Load Unfiltered LoRA 5K
    print("\n2. Loading Unfiltered LoRA 5K...", flush=True)
    tokenizer = AutoTokenizer.from_pretrained("google/flan-t5-base")
    base_model = AutoModelForSeq2SeqLM.from_pretrained("google/flan-t5-base")
    
    unfiltered_path = BASE_DIR / "final_models/unfiltered_lora_5k"
    if not unfiltered_path.exists():
        print(f"❌ Unfiltered LoRA 5K not found: {unfiltered_path}", flush=True)
        print("   Must train Unfiltered LoRA 5K first!", flush=True)
        return
    
    model = PeftModel.from_pretrained(base_model, str(unfiltered_path))
    ...
    with open(training_data_dir / "unfiltered_5k_train_prompts.txt", 'r') as f:
    ...
    training_args = TrainingArguments(
        output_dir=str(BASE_DIR / "final_models/unfiltered_lora_5k_rl"),
        num_train_epochs=1,
    ...
    trainer = RLTrainer(
        ...
        tokenizer=tokenizer,
        callbacks=[GitHubPushCallback(BASE_DIR, "unfiltered_lora_5k_rl")]
    )
    
    print("\n5. Training with RL...", flush=True)
    print("   Expected time: 45-60 minutes", flush=True)
    print("   Baseline: Unfiltered 5K has 47.9% LCR", flush=True)
    print("   Goal: Push it higher with RL!\n", flush=True)
    sys.stdout.flush()
    
    trainer.train()
    
    print("\n6. Saving final model...", flush=True)
    model.save_pretrained(BASE_DIR / "final_models/unfiltered_lora_5k_rl")
    tokenizer.save_pretrained(BASE_DIR / "final_models/unfiltered_lora_5k_rl")
    
    print("\n7. Final push to GitHub...", flush=True)
    subprocess.run(["git", "pull", "--rebase"], cwd=BASE_DIR, capture_output=True)
    subprocess.run(["git", "add", "final_models/unfiltered_lora_5k_rl/"], cwd=BASE_DIR)
    subprocess.run(["git", "commit", "-m", "Final: Unfiltered LoRA 5K + RL complete"], cwd=BASE_DIR, capture_output=True)
    
    for attempt in range(3):
        result = subprocess.run(["git", "push"], cwd=BASE_DIR, capture_output=True, text=True, timeout=30)
        if result.returncode == 0:
