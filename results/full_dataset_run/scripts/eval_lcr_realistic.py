...
GENERATIONS_PATH = BASE_DIR / "outputs" / "baseline_realistic_generations.csv"
KEYWORDS_PATH = BASE_DIR / "evaluation" / "keywords.json"
SCORED_OUT_PATH = BASE_DIR / "outputs" / "baseline_realistic_generations_scored.csv"
SUMMARY_OUT_PATH = BASE_DIR / "evaluation" / "lcr_baseline_realistic.csv"
...
def main():
    ...
    if "generation" not in df.columns:
        raise ValueError("Expected a 'generation' column in baseline_realistic_generations.csv")
    ...
    print("\n=== BASELINE REALISTIC TEST RESULTS ===")
    print(f"Total: {total}")
    print(f"No forbidden: {summary['no_forbidden']} ({summary['no_forbidden']/total*100:.1f}%)")
    print(f"Has required: {summary['has_required']} ({summary['has_required']/total*100:.1f}%)")
    print(f"Compliant: {summary['compliant']} ({summary['compliant']/total*100:.1f}%)")
